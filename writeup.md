...
Applying 3 preprocessing techniques to the image data in the following order: histogram equalization, grayscaling, and normalization.
...
The images seem relatively similar to the training dataset. After the preprocessing step of the new images, they are very much unaffected by resolution, brightness, clutter, blur, etc . Let's see however how it is performing with them.
...
The Accuracy for the Web Images are almost as same as the test iamges, which proves the model is pretty confident in detecting the Traffic signs.
